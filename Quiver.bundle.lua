...
__bundle_register("Diagnostics.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
-- Quiver Octo5 stabilization diagnostic instrumentation.
-- This module is intentionally observation-only: it records ClassicAPI events,
-- selected API snapshots, and Quiver state transitions without changing the
-- decisions made by the normal modules.

local BUILD = "3.1.5-Octo5-Diag1"
local SCHEMA = 2
local MAX_RECORDS = 20000
local TRIM_RECORDS = 5000

local pending = {}
local store = nil
local customRegistered = false
local oldErrorHandler = nil
local errorHookInstalled = false
local sessionId = 0

local scalar = function(v)
	local t = type(v)
...
local ensureStore = function()
	...
	Quiver_Diagnostics = Quiver_Diagnostics or {}
	store = Quiver_Diagnostics
	-- Do not mix traces from a previous diagnostic build with this one.
	if store.Build ~= nil and store.Build ~= BUILD then
		store.Records = {}
		store.Dropped = 0
		-- A new diagnostic build starts recording automatically even if the
		-- previous build had been stopped manually.
		store.Active = true
	end
	store.Schema = SCHEMA
	store.Build = BUILD
	if store.Active == nil then store.Active = true end
	store.Records = store.Records or {}
	store.Dropped = tonumber(store.Dropped) or 0
	store.LastSessionStart = store.LastSessionStart or 0
	store.SessionCounter = tonumber(store.SessionCounter) or 0
	if table.getn(pending) > 0 then
		for _i, rec in ipairs(pending) do table.insert(store.Records, rec) end
		pending = {}
	end
	return store
end

local append = function(rec, force)
	local s = store
	if s == nil then
		if force or table.getn(pending) < 200 then table.insert(pending, rec) end
		return
	end
	if not force and not s.Active then return end
	local records = s.Records
	if table.getn(records) >= MAX_RECORDS then
		-- Keep a rolling trace. Dropping the newest records would make an
		-- intermittent bug that happens late in a long raid impossible to
		-- diagnose. Trim in a large chunk so this O(n) copy is rare.
		local n = table.getn(records)
		local firstKeep = math.min(n + 1, TRIM_RECORDS + 1)
		local fresh = {}
		for i=firstKeep,n do table.insert(fresh, records[i]) end
		s.Dropped = (s.Dropped or 0) + firstKeep - 1
		s.Records = fresh
		records = fresh
	end
...
local trace = function(kind, data, force)
	local rec = { t = GetTime and GetTime() or 0, kind = kind, session = sessionId }
	if data ~= nil then
		for k, v in pairs(data) do rec[k] = scalar(v) end
	end
	append(rec, force == true)
end

local snapshotCast = function(reason)
	if type(C_Spell) ~= "table" or type(C_Spell.UnitCastingInfo) ~= "function" then
		trace("CAST_SNAPSHOT_UNAVAILABLE", { reason=reason })
		return
	end
	local ok, name, rank, texture, startMs, endMs, tradeskill, castGuid, notInterruptible, spellId =
		pcall(C_Spell.UnitCastingInfo, "player")
	if not ok then
		trace("CAST_SNAPSHOT_ERROR", { reason=reason, error=name })
		return
	end
	trace("CAST_SNAPSHOT", {
		reason=reason, name=name, rank=rank, texture=texture,
		startMs=startMs, endMs=endMs, tradeskill=tradeskill,
		castGuid=castGuid, notInterruptible=notInterruptible, spellId=spellId,
	})
end

local snapshotMovement = function(reason)
	if type(GetUnitSpeed) ~= "function" then
		trace("MOVEMENT_SNAPSHOT_UNAVAILABLE", { reason=reason })
		return
	end
	local ok, current, run, flight, swim = pcall(GetUnitSpeed, "player")
	if not ok then
		trace("MOVEMENT_SNAPSHOT_ERROR", { reason=reason, error=current })
		return
	end
	trace("MOVEMENT_SNAPSHOT", { reason=reason, current=current, run=run, flight=flight, swim=swim })
end

local snapshotTarget = function(reason)
	local exists = UnitExists("target") and true or false
	trace("TARGET_SNAPSHOT", {
		reason=reason,
		exists=exists,
		dead=exists and (UnitIsDead("target") and true or false) or false,
		attackable=exists and (UnitCanAttack("player", "target") and true or false) or false,
		creatureType=exists and UnitCreatureType("target") or nil,
		level=exists and UnitLevel("target") or nil,
		health=exists and UnitHealth("target") or nil,
		healthMax=exists and UnitHealthMax("target") or nil,
	})
end

local selfTest = function()
	trace("SELFTEST_BEGIN", { classicApiVersion=CLASSIC_API_VERSION, locale=GetLocale and GetLocale() or nil }, true)
	local checks = {
		{ "GetUnitSpeed", type(GetUnitSpeed) },
		{ "C_Spell", type(C_Spell) },
		{ "C_UnitAuras", type(C_UnitAuras) },
		{ "C_CreatureInfo", type(C_CreatureInfo) },
	}
	for _i, c in ipairs(checks) do trace("SELFTEST_TYPE", { name=c[1], value=c[2] }, true) end

	if type(C_Spell) == "table" then
		trace("SELFTEST_TYPE", { name="C_Spell.UnitCastingInfo", value=type(C_Spell.UnitCastingInfo) }, true)
		trace("SELFTEST_TYPE", { name="C_Spell.IsSpellInRange", value=type(C_Spell.IsSpellInRange) }, true)
		trace("SELFTEST_TYPE", { name="C_Spell.IsRangedAutoAttackSpell", value=type(C_Spell.IsRangedAutoAttackSpell) }, true)
		trace("SELFTEST_TYPE", { name="C_Spell.GetSpellName", value=type(C_Spell.GetSpellName) }, true)
		if type(C_Spell.GetSpellName) == "function" then
			for _i, sid in ipairs({75,19434,3035,19801,19506,45650,45651}) do
				local okName, spellName = pcall(C_Spell.GetSpellName, sid)
				trace("SELFTEST_SPELL_NAME", { spellId=sid, ok=okName, name=spellName }, true)
			end
		end
		if type(C_Spell.IsRangedAutoAttackSpell) == "function" then
			local ok1, a = pcall(C_Spell.IsRangedAutoAttackSpell, 75)
			local ok2, b = pcall(C_Spell.IsRangedAutoAttackSpell, 19434)
			trace("SELFTEST_AUTOREPEAT", { autoShotOk=ok1, autoShot=a, aimedOk=ok2, aimed=b }, true)
		end
	end
	if type(C_CreatureInfo) == "table" and type(C_CreatureInfo.GetCreatureTypeInfo) == "function" then
		local ok, info = pcall(C_CreatureInfo.GetCreatureTypeInfo, 1)
		trace("SELFTEST_CREATURE_TYPE", { ok=ok, name=ok and info and info.name or nil }, true)
	end
	snapshotCast("selftest")
	snapshotMovement("selftest")
	snapshotTarget("selftest")
	trace("SELFTEST_END", {}, true)
end

local registerEventSafe = function(frame, name)
	local ok, err = pcall(frame.RegisterEvent, frame, name)
	trace("EVENT_REGISTER", { event=name, ok=ok, error=ok and nil or err }, true)
	return ok
end
...
local registerCustomEvents = function(frame)
	if customRegistered then return end
	if type(CLASSIC_API_VERSION) ~= "number" or CLASSIC_API_VERSION < 11400 then
		trace("CUSTOM_EVENT_REGISTRATION_DEFERRED", { reason="ClassicAPI 1.14.0+ unavailable", version=CLASSIC_API_VERSION }, true)
		return
	end
	if type(C_EventUtils) ~= "table" or type(C_EventUtils.IsEventValid) ~= "function" then
		trace("CUSTOM_EVENT_REGISTRATION_DEFERRED", { reason="C_EventUtils.IsEventValid unavailable" }, true)
		return
	end
	local allOk = true
	for _i, name in ipairs(customEvents) do
		local okValid, valid = pcall(C_EventUtils.IsEventValid, name)
		trace("EVENT_VALID", { event=name, ok=okValid, valid=valid }, true)
		if not okValid or valid ~= true or not registerEventSafe(frame, name) then allOk = false end
	end
	customRegistered = allOk
end

local installErrorHook = function()
	if errorHookInstalled then return end
	if type(geterrorhandler) ~= "function" or type(seterrorhandler) ~= "function" then
		trace("ERROR_HOOK_UNAVAILABLE", {}, true)
		return
	end
	oldErrorHandler = geterrorhandler()
	if type(oldErrorHandler) ~= "function" then return end
	local handler = function(msg)
		trace("LUA_ERROR", { message=msg }, true)
		return oldErrorHandler(msg)
	end
	seterrorhandler(handler)
	errorHookInstalled = true
	trace("ERROR_HOOK_INSTALLED", {}, true)
end

local frame = CreateFrame("Frame", nil)
local vanillaEvents = {
	"VARIABLES_LOADED", "PLAYER_LOGIN", "PLAYER_LOGOUT", "PLAYER_ENTERING_WORLD",
	"PLAYER_TARGET_CHANGED", "PLAYER_AURAS_CHANGED", "UNIT_FLAGS", "UNIT_FACTION",
	"UNIT_HEALTH", "SPELLS_CHANGED", "PLAYER_REGEN_DISABLED", "PLAYER_REGEN_ENABLED",
	"START_AUTOREPEAT_SPELL", "STOP_AUTOREPEAT_SPELL",
}
for _i, name in ipairs(vanillaEvents) do registerEventSafe(frame, name) end
...
frame:SetScript("OnUpdate", function()
	...
	movementElapsed = movementElapsed + elapsed
	perfElapsed = perfElapsed + elapsed
	if movementElapsed >= 0.10 then
		movementElapsed = 0
		if type(GetUnitSpeed) == "function" then
			local speed = GetUnitSpeed("player") or 0
			local moving = speed > 0.01
			if lastMoving == nil or moving ~= lastMoving or lastSpeed == nil or math.abs(speed-lastSpeed) >= 0.25 then
				trace("MOVEMENT_CHANGE", { moving=moving, speed=speed })
				lastMoving = moving
				lastSpeed = speed
			end
		end
	end
	if perfElapsed >= 5 then
		perfElapsed = 0
		local fps = type(GetFramerate)=="function" and GetFramerate() or nil
		local home, world = nil, nil
		if type(GetNetStats)=="function" then
			local _down, _up, h, w = GetNetStats()
			home, world = h, w
		end
		trace("PERF", { fps=fps, homeLatency=home, worldLatency=world })
	end
end)
...
frame:SetScript("OnEvent", function()
	...
	if ev == "VARIABLES_LOADED" then
		ensureStore()
		-- Diagnostic candidates always begin a fresh session recording.
		-- /qdiag stop remains useful for the current session, but cannot leave
		-- a future player report silently unrecorded after /reload or restart.
		store.Active = true
		store.SessionCounter = (store.SessionCounter or 0) + 1
		sessionId = store.SessionCounter
		store.LastSessionStart = GetTime()
		trace("SESSION_START", {
			build=BUILD,
			classicApiVersion=CLASSIC_API_VERSION,
			...
			uiScale=GetCVar and GetCVar("uiScale") or nil,
		}, true)
		installErrorHook()
		registerCustomEvents(frame)
	elseif ev == "PLAYER_LOGIN" then
		ensureStore()
		registerCustomEvents(frame)
		trace("PLAYER_LOGIN", { classicApiVersion=CLASSIC_API_VERSION }, true)
		selfTest()
	elseif ev == "PLAYER_LOGOUT" then
		trace("PLAYER_LOGOUT", { records=store and table.getn(store.Records) or 0, dropped=store and store.Dropped or 0 }, true)
	elseif ev == "PLAYER_TARGET_CHANGED" then
		trace("EVENT", { event=ev })
		snapshotTarget(ev)
	elseif string.find(ev or "", "UNIT_SPELLCAST_", 1, true) == 1 then
		trace("SPELLCAST_EVENT", { event=ev, a1=arg1, a2=arg2, a3=arg3, a4=arg4, a5=arg5, a6=arg6 })
		snapshotCast(ev)
		snapshotMovement(ev)
	elseif ev == "START_AUTOREPEAT_SPELL" or ev == "STOP_AUTOREPEAT_SPELL" then
		trace("AUTOREPEAT_EVENT", { event=ev })
		snapshotMovement(ev)
	else
		trace("EVENT", { event=ev, a1=arg1, a2=arg2, a3=arg3 })
	end
end)

SLASH_QUIVERDIAG1 = "/qdiag"
SlashCmdList["QUIVERDIAG"] = function(msg)
	ensureStore()
	msg = msg or ""
	...
	if cmd == "start" then
		...
		store.Dropped = 0
		store.Active = true
		trace("DIAG_START", { build=BUILD, classicApiVersion=CLASSIC_API_VERSION }, true)
		DEFAULT_CHAT_FRAME:AddMessage("|cff88ccffQuiverDiag: recording started.|r")
		selfTest()
	elseif cmd == "stop" then
		trace("DIAG_STOP", { records=table.getn(store.Records), dropped=store.Dropped }, true)
		store.Active = false
		DEFAULT_CHAT_FRAME:AddMessage("|cff88ccffQuiverDiag: recording stopped. /reload or exit WoW to write SavedVariables.|r")
	elseif cmd == "clear" then
		store.Records = {}
		store.Dropped = 0
		DEFAULT_CHAT_FRAME:AddMessage("|cff88ccffQuiverDiag: log cleared.|r")
	elseif cmd == "mark" then
		trace("MARK", { text=rest }, true)
		DEFAULT_CHAT_FRAME:AddMessage("|cff88ccffQuiverDiag marker:|r "..rest)
	elseif cmd == "selftest" then
		selfTest()
		DEFAULT_CHAT_FRAME:AddMessage("|cff88ccffQuiverDiag: self-test recorded.|r")
	elseif cmd == "snapshot" then
		snapshotCast("manual")
		snapshotMovement("manual")
		snapshotTarget("manual")
		DEFAULT_CHAT_FRAME:AddMessage("|cff88ccffQuiverDiag: snapshot recorded.|r")
	elseif cmd == "status" then
		DEFAULT_CHAT_FRAME:AddMessage("|cff88ccffQuiverDiag:|r active="..tostring(store.Active).." records="..tostring(table.getn(store.Records)).." dropped="..tostring(store.Dropped))
	else
		DEFAULT_CHAT_FRAME:AddMessage("|cff88ccffQuiverDiag:|r /qdiag start | stop | mark <text> | selftest | snapshot | status | clear")
	end
end

return {
	Build = BUILD,
	Trace = function(kind, data) trace(kind, data, false) end,
	TraceAlways = function(kind, data) trace(kind, data, true) end,
	SnapshotCast = snapshotCast,
	SnapshotMovement = snapshotMovement,
	SnapshotTarget = snapshotTarget,
}
end)
__bundle_register("__root", function(require, _LOADED, __bundle_register, __bundle_modules)
local Diag = require "Diagnostics.wow.lua"
local Api = require "Api/Index.wow.lua"
local MainMenu = require "Config/MainMenu.wow.lua"
local LoadLocale = require "Locale/Lang.wow.lua"
local Migrations = require "Migrations/Runner.wow.lua"
local AspectTracker = require "Modules/Aspect_Tracker/AspectTracker.wow.lua"
local AutoShotTimer = require "Modules/Auto_Shot_Timer/AutoShotTimer.wow.lua"
local Castbar = require "Modules/Castbar.wow.lua"
local RangeIndicator = require "Modules/RangeIndicator.wow.lua"
local TranqAnnouncer = require "Modules/TranqAnnouncer.wow.lua"
local TrueshotAuraAlarm = require "Modules/TrueshotAuraAlarm.wow.lua"
local RegisterGlobalFunctions = require "GlobalFunctions.wow.lua"

_G = _G or getfenv()
Quiver = Quiver or {}
_G.Quiver_Modules = { AspectTracker, AutoShotTimer, Castbar, RangeIndicator, TranqAnnouncer, TrueshotAuraAlarm }

local MIN_CLASSIC_API_VERSION = 11400
local REQUIRED_CUSTOM_EVENTS = {
	"UNIT_SPELLCAST_START",
	"UNIT_SPELLCAST_DELAYED",
	"UNIT_SPELLCAST_STOP",
	"UNIT_SPELLCAST_SUCCEEDED",
...
-- Release contract: require the exact ClassicAPI generation this fork was
-- audited against, then only the APIs/events that active Quiver code uses.
local getClassicApiProblem = function()
	if type(CLASSIC_API_VERSION) ~= "number" then return "CLASSIC_API_VERSION" end
	if CLASSIC_API_VERSION < MIN_CLASSIC_API_VERSION then
		return "ClassicAPI 1.14.0+ (found "..tostring(CLASSIC_API_VERSION)..")"
	end
	if type(GetUnitSpeed) ~= "function" then return "GetUnitSpeed" end
	if type(C_Spell) ~= "table" then return "C_Spell" end
	if type(C_Spell.UnitCastingInfo) ~= "function" then return "C_Spell.UnitCastingInfo" end
	if type(C_Spell.IsSpellInRange) ~= "function" then return "C_Spell.IsSpellInRange" end
	if type(C_Spell.IsRangedAutoAttackSpell) ~= "function" then return "C_Spell.IsRangedAutoAttackSpell" end
	if type(C_Spell.GetSpellName) ~= "function" then return "C_Spell.GetSpellName" end
	if type(C_UnitAuras) ~= "table" then return "C_UnitAuras" end
	if type(C_UnitAuras.GetAuraDataBySpellName) ~= "function" then return "C_UnitAuras.GetAuraDataBySpellName" end
	if type(C_CreatureInfo) ~= "table" then return "C_CreatureInfo" end
	if type(C_CreatureInfo.GetCreatureTypeInfo) ~= "function" then return "C_CreatureInfo.GetCreatureTypeInfo" end
	if type(C_EventUtils) ~= "table" or type(C_EventUtils.IsEventValid) ~= "function" then
		return "C_EventUtils.IsEventValid"
	end
	for _i, eventName in ipairs(REQUIRED_CUSTOM_EVENTS) do
		local ok, valid = pcall(C_EventUtils.IsEventValid, eventName)
		if not ok or valid ~= true then return "ClassicAPI event "..eventName end
	end
	return nil
end
...
local frameConfigMenu = nil
local initSlashCommandsAndModules = function()
	local _, cl = UnitClass("player")
	if cl == "HUNTER" then
		frameConfigMenu = MainMenu.Create("QuiverConfigDialog")
		Api.Aero.RegisterFrame(frameConfigMenu)
		SlashCmdList["QUIVER"] = function() frameConfigMenu:Show() end
		for _i, v in ipairs(_G.Quiver_Modules) do
			if Quiver_Store.ModuleEnabled[v.Id] then v.OnEnable() end
		end
	else
		SlashCmdList["QUIVER"] = function()
			DEFAULT_CHAT_FRAME:AddMessage(Quiver.T["Quiver is for hunters."], 1, 0.5, 0)
		end
	end
end
...
SlashCmdList["QUIVER"] = function()
	DEFAULT_CHAT_FRAME:AddMessage("|cffff3333Quiver Octo: waiting for ClassicAPI 1.14.0+; Quiver has not initialized yet.|r")
end

local initialized = false
local warnedMissing = false
local attemptInitialize = function(finalAttempt)
	if initialized then return true end
	local problem = getClassicApiProblem()
	Diag.TraceAlways("QUIVER_INIT_ATTEMPT", {
		final=finalAttempt, problem=problem, classicApiVersion=CLASSIC_API_VERSION,
	})
	if problem ~= nil then
		if finalAttempt and not warnedMissing then
			warnedMissing = true
			DEFAULT_CHAT_FRAME:AddMessage("|cffff3333Quiver Octo: ClassicAPI contract failed: "..problem..". Quiver was not initialized.|r")
		end
		return false
	end

	-- No ClassicAPI custom event is registered by a Quiver feature before this
	-- contract has passed. Module OnEnable handlers do their event registration.
	LoadLocale()
	Migrations()
	...
	initialized = true
	Diag.TraceAlways("QUIVER_INIT_SUCCESS", { classicApiVersion=CLASSIC_API_VERSION })
	return true
end
...
frame:SetScript("OnEvent", function()
	...
	elseif event == "PLAYER_LOGOUT" and initialized then
		savedVariablesPersist()
	end
end)
end)
__bundle_register("GlobalFunctions.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Api = require "Api/Index.wow.lua"
local AutoShotTimer = require "Modules/Auto_Shot_Timer/AutoShotTimer.wow.lua"
local TrinketSwap = require "Modules/TrinketSwap.wow.lua"
...
local FdPrepareTrap = function()
	...
	local trap = Api.Spell.GetLocalizedName(Api.Spell.Id.ImmolationTrap, "Immolation Trap")
	local fd = Api.Spell.GetLocalizedName(Api.Spell.Id.FeignDeath, "Feign Death")
...
return function()
	...
	-- Safer WIP macro API: prefer an item ID or item link. Legacy texture paths
	-- are accepted only when that icon maps to one unique item ID in the bags.
	-- /run Quiver.TrinketSwap1(23041)
	-- /run Quiver.TrinketSwap2("|Hitem:23041:0:0:0|h[Slayer's Crest]|h")
	Quiver.TrinketSwap1 = function(item) TrinketSwap("Trinket0Slot", item) end
	Quiver.TrinketSwap2 = function(item) TrinketSwap("Trinket1Slot", item) end
end
end)
__bundle_register("Modules/TrinketSwap.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Api = require "Api/Index.wow.lua"
local Diag = require "Diagnostics.wow.lua"

local _BAGS={0,1,2,3,4}
local SID=Api.Spell.Id

local getContainerItemId=function(bagId,slot)
...
local findBagSlotByItemId=function(itemId)
	for _i,bagId in ipairs(_BAGS) do
		local numSlots=GetContainerNumSlots(bagId)
		for slot=1,numSlots do
			if getContainerItemId(bagId,slot)==itemId then
				local _texture,_count,locked=GetContainerItemInfo(bagId,slot)
				return bagId,slot,locked and true or false
			end
		end
	end
...
-- Backward-compatible resolver for the old undocumented texture macro. A
-- texture is accepted only when it maps to exactly one item ID in the bags;
-- ambiguous icons are refused instead of equipping an arbitrary item.
local resolveRequest=function(request)
	if type(request)=="number" then
		local id=math.floor(request)
		return id>0 and id or nil,"itemId"
	end
	if type(request)~="string" then return nil,"invalid" end
	local numeric=tonumber(request)
	if numeric and numeric>0 then return math.floor(numeric),"numericString" end
	local _,_,linked=string.find(request,"item:(%d+)")
	if linked then return tonumber(linked),"itemLink" end
	...
	for _i,bagId in ipairs(_BAGS) do
		for slot=1,GetContainerNumSlots(bagId) do
			...
			if texture==request then
				local id=getContainerItemId(bagId,slot)
				if id and not unique[id] then unique[id]=true; countUnique=countUnique+1 end
			end
		end
	end
	if countUnique==1 then
		for id,_ in pairs(unique) do return id,"uniqueTexture" end
	elseif countUnique>1 then
		DEFAULT_CHAT_FRAME:AddMessage("|cffff6633Quiver Trinket: that icon matches multiple item IDs; use an item ID or item link.|r")
		Diag.Trace("TRINKET_AMBIGUOUS_TEXTURE", { texture=request, unique=countUnique })
		return nil,"ambiguousTexture"
	end
	return nil,"notFound"
end

local TrinketSwap=function(slotName,request)
	local itemId,source=resolveRequest(request)
	Diag.Trace("TRINKET_RESOLVE", { request=request, itemId=itemId, source=source })
	if itemId==nil then
		if source~="ambiguousTexture" then DEFAULT_CHAT_FRAME:AddMessage("Quiver Trinket Failure: item not found; use an item ID or item link.") end
		return
	end

	local trinketSlot=GetInventorySlotInfo(slotName)
	local bagId,slot,bagItemLocked=findBagSlotByItemId(itemId)
	Diag.Trace("TRINKET_REQUEST", { slotName=slotName, itemId=itemId, bagId=bagId, bagSlot=slot, bagLocked=bagItemLocked, cursor=CursorHasItem() and true or false, targeting=SpellIsTargeting() and true or false, invLocked=IsInventoryItemLocked(trinketSlot) and true or false, combat=UnitAffectingCombat("player") and true or false })
	if bagId==nil or slot==nil or bagItemLocked or CursorHasItem() or SpellIsTargeting() or IsInventoryItemLocked(trinketSlot) then return end
	if UnitAffectingCombat("player") then
		if UnitExists("pettarget") and UnitAffectingCombat("pet") then PetPassiveMode(); PetFollow() end
		Diag.Trace("TRINKET_FEIGN_DEATH", { itemId=itemId, bagId=bagId, bagSlot=slot })
		local fd=Api.Spell.GetLocalizedName(SID.FeignDeath,"Feign Death")
		if fd then CastSpellByName(fd) end
		return
	end
	Diag.Trace("TRINKET_SWAP", { itemId=itemId, bagId=bagId, bagSlot=slot, inventorySlot=trinketSlot })
	PickupContainerItem(bagId,slot)
	PickupInventoryItem(trinketSlot)
	-- If the target trinket slot was occupied, the old trinket is now on
	-- the cursor. Put it back into the exact bag slot vacated by the new item
	-- so the helper finishes the swap instead of leaving an item on cursor.
	if CursorHasItem() then
		Diag.Trace("TRINKET_RETURN_OLD", { bagId=bagId, bagSlot=slot, inventorySlot=trinketSlot })
		PickupContainerItem(bagId,slot)
	end
end
return TrinketSwap
end)
__bundle_register("Modules/Auto_Shot_Timer/AutoShotTimer.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Api = require "Api/Index.wow.lua"
local Const = require "Constants.pure.lua"
local FrameLock = require "Events/FrameLock.wow.lua"
local BorderStyle = require "Modules/BorderStyle.state.lua"
local Print = require "Util/Print.wow.lua"
local Diag = require "Diagnostics.wow.lua"

local _AIMING_TIME = 0.5
local _MOVE_EPSILON = 0.01
...
local isCastingShot = false
local currentCastGuid = nil

local traceState = function(reason, spellId, guid)
	Diag.Trace("AUTOSHOT_STATE", {
		reason=reason, autoRepeat=isAutoRepeat, reloading=isReloading, castingShot=isCastingShot,
		currentCastGuid=currentCastGuid, spellId=spellId, guid=guid,
	})
end

local playerMoving = function()
	local speed = GetUnitSpeed("player") or 0
	return speed > _MOVE_EPSILON
end
...
local timeReload = (function()
	...
	return {
		GetPercentCompleted = function()
			if reloadTime <= 0 then return 1 end
			return math.min(1, getElapsed() / reloadTime)
		end,
		...
		StartAt = function(t)
			...
			reloadTime = math.max(0.01, (tonumber(speed) or _AIMING_TIME) - _AIMING_TIME)
			isReloading = true
			Print.Debug("starting authoritative reload")
		end,
	}
end)()
...
local timeShoot = (function()
	...
	return {
		GetPercentCompleted = function()
			return math.min(1, getElapsed() / _AIMING_TIME)
		end,
...
local styleBarAutoShot = function(f)
	...
	maxBarWidth = math.max(1, f:GetWidth() - 2 * sizeInset)
	f.BarAutoShot:SetWidth(1)
	f.BarAutoShot:SetHeight(math.max(1, f:GetHeight() - 2 * sizeInset))
end

local setFramePosition = function(f, s)
	FrameLock.SideEffectRestoreSize(s, { w=240, h=14, dx=240 * -0.5, dy=-136 })
...
local createUI = function()
	...
	f.BarAutoShot:SetBackdrop({ bgFile = "Interface/BUTTONS/WHITE8X8", tile = false })
...
local updateBarShooting = function()
	...
	if isCastingShot then
		frame.BarAutoShot:SetWidth(1)
	else
		frame.BarAutoShot:SetWidth(math.max(1, maxBarWidth * timeShoot.GetPercentCompleted()))
	end
end
local updateBarReload = function()
	...
	frame.BarAutoShot:SetBackdropColor(r, g, b, 0.8)
	local completed = timeReload.GetPercentCompleted()
	if completed < 1 then
		frame.BarAutoShot:SetWidth(math.max(1, maxBarWidth * (1 - completed)))
	else
		isReloading = false
		Print.Debug("End authoritative reload")
...
local handleUpdate = function()
	if isReloading then
		updateBarReload()
	elseif isAutoRepeat and not playerMoving() then
		updateBarShooting()
	else
		-- Auto Shot requires a fresh aim after movement stops.
		if isAutoRepeat and playerMoving() then timeShoot.Reset() end
		frame.BarAutoShot:SetWidth(1)
		tryHideBar()
	end
end

local predTrackedCastedShot = function(nameLocalized)
	local nameEnglish = Api.Spell.GetEnglishName(nameLocalized)
	local meta = nameEnglish and Api.Spell.Db[nameEnglish] or nil
	return meta ~= nil and not Api.Spell.PredInstantCast(nameEnglish)
end

-- ************ Authoritative ClassicAPI events ************
local _EVENTS = {
...
local finishTrackedCast = function(guid)
	if isCastingShot and guid == currentCastGuid then
		isCastingShot = false
		currentCastGuid = nil
		if isAutoRepeat and not isReloading then timeShoot.Reset() end
		traceState("CAST_FINISH", nil, guid)
	end
end
...
local handleEvent = function()
	if event == "START_AUTOREPEAT_SPELL" then
		...
		startShooting()
		traceState("START_AUTOREPEAT_SPELL")
	elseif event == "STOP_AUTOREPEAT_SPELL" then
		Print.Debug("Stop shooting")
		isAutoRepeat = false
		traceState("STOP_AUTOREPEAT_SPELL")
		if not isReloading then tryHideBar() end
	elseif event == "UNIT_SPELLCAST_START" then
		if arg1 == "player" and predTrackedCastedShot(arg4) then
			isCastingShot = true
			currentCastGuid = arg2
			Print.Debug("Start casted shot")
			traceState("UNIT_SPELLCAST_START", arg3, arg2)
		end
	...
	then
		if arg1 == "player" then finishTrackedCast(arg2) end
	elseif event == "UNIT_SPELLCAST_SUCCEEDED" and arg1 == "player" then
		local spellId = arg3
		local isRangedAuto = spellId ~= nil and C_Spell.IsRangedAutoAttackSpell(spellId)
		Diag.Trace("AUTOSHOT_CLASSIFY", { spellId=spellId, guid=arg2, isRangedAuto=isRangedAuto })
		if isRangedAuto then
			-- The server confirmed a real Auto Shot. No inventory-lock inference,
			-- macro-texture guessing, or FPS-sensitive event ordering is involved.
			isAutoRepeat = true
			timeReload.StartAt(GetTime())
			traceState("AUTO_SHOT_SUCCEEDED", spellId, arg2)
		else
			-- Successful non-auto spells reset the 0.5 s aiming phase when we are
			-- between shots, matching the observed Vanilla/Turtle behavior.
			if isAutoRepeat and not isReloading and not isCastingShot then
				timeShoot.Reset()
			end
		end
	end
...
local PredMidShot = function()
	return isAutoRepeat and not isReloading and not isCastingShot and not playerMoving()
end
local GetSecondsRemainingReload = function()
	if isReloading then return true, timeReload.GetRemaining() end
	return false, 0
end
local GetSecondsRemainingShoot = function()
	if isAutoRepeat and not isReloading and not isCastingShot and not playerMoving() then
		return true, timeShoot.GetRemaining()
	end
	return false, 0
end
...
return {
	...
	OnInterfaceLock = function()
		if (not isAutoRepeat) and (not isReloading) then tryHideBar() end
	end,
	OnInterfaceUnlock = function() if frame ~= nil then frame:SetAlpha(1) end end,
	OnResetFrames = function()
		store.FrameMeta = nil
		if frame then setFramePosition(frame, store); styleBarAutoShot(frame) end
	end,
	...
	UpdateDirection = function() if frame then styleBarAutoShot(frame) end end,
...
__bundle_register("Util/Print.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
...
__bundle_register("Util/Haste.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
...
__bundle_register("Lib/Index.pure.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Array = require("Lib/Array.pure.lua")
local Color = require("Lib/Color.pure.lua")
local M = require("Lib/Monoid.pure.lua")
local Nil = require("Lib/Nil.pure.lua")
local Sg = require("Lib/Semigroup.pure.lua")
...
__bundle_register("Lib/Semigroup.pure.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Semigroup = {}
...
__bundle_register("Lib/Nil.pure.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Nil = {}
...
__bundle_register("Lib/Monoid.pure.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Monoid = {}
...
__bundle_register("Lib/Color.pure.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
...
__bundle_register("Lib/Array.pure.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local M = require("Lib/Monoid.pure.lua")
...
__bundle_register("Constants.pure.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
return {
...
__bundle_register("Api/Index.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
...
__bundle_register("Api/Tooltip.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local L = require("Lib/Index.pure.lua")
...
__bundle_register("Api/Spell.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
---@class SpellMetaAll
...
-- Stable/base spell IDs used only to resolve the client's localized spell name,
-- or directly with ClassicAPI calls that accept spell IDs. Rank 1 is enough for
-- name resolution because ranks share the same localized name.
local ID = {
	AimedShot = 19434,
	MultiShot = 2643,
	SteadyShot = 3035, -- Turtle WoW Steady Shot rank 1
	ArcaneShot = 3044,
	ConcussiveShot = 5116,
	ScatterShot = 19503,
	ScorpidSting = 3043,
	...
	AspectBeast = 13161,
	AspectCheetah = 5118,
	AspectFox = 45651, -- Turtle WoW custom spell
	AspectHawk = 13165,
	AspectMonkey = 13163,
	AspectPack = 13159,
	AspectViper = 34074,
	AspectWild = 20043,
	AspectWolf = 45650, -- Turtle WoW custom spell (rank 1)
}
...
local ENGLISH_TO_ID = {
	["Aimed Shot"] = ID.AimedShot,
	["Multi-Shot"] = ID.MultiShot,
	["Steady Shot"] = ID.SteadyShot,
	["Arcane Shot"] = ID.ArcaneShot,
	["Concussive Shot"] = ID.ConcussiveShot,
	["Scatter Shot"] = ID.ScatterShot,
	["Scorpid Sting"] = ID.ScorpidSting,
	["Serpent Sting"] = ID.SerpentSting,
	["Viper Sting"] = ID.ViperSting,
	["Wyvern Sting"] = ID.WyvernSting,
	["Auto Shot"] = ID.AutoShot,
	["Wing Clip"] = ID.WingClip,
	["Hunter's Mark"] = ID.HuntersMark,
	["Scare Beast"] = ID.ScareBeast,
	["Tranquilizing Shot"] = ID.TranquilizingShot,
	["Trueshot Aura"] = ID.TrueshotAura,
	["Feign Death"] = ID.FeignDeath,
	["Immolation Trap"] = ID.ImmolationTrap,
	["Aspect of the Beast"] = ID.AspectBeast,
	["Aspect of the Cheetah"] = ID.AspectCheetah,
	["Aspect of the Fox"] = ID.AspectFox,
	["Aspect of the Hawk"] = ID.AspectHawk,
	["Aspect of the Monkey"] = ID.AspectMonkey,
	["Aspect of the Pack"] = ID.AspectPack,
	["Aspect of the Viper"] = ID.AspectViper,
	["Aspect of the Wild"] = ID.AspectWild,
	["Aspect of the Wolf"] = ID.AspectWolf,
}

local localizedNameCache = {}
local localizedToEnglishCache = {}

---@param spellId integer
---@param fallbackEnglish? string
---@return nil|string
local GetLocalizedName = function(spellId, fallbackEnglish)
	if localizedNameCache[spellId] ~= nil then return localizedNameCache[spellId] end
	local name = nil
	if type(C_Spell) == "table" and type(C_Spell.GetSpellName) == "function" then
		name = C_Spell.GetSpellName(spellId)
	end
	if type(name) ~= "string" or name == "" then
		if fallbackEnglish ~= nil and Quiver and Quiver.L and Quiver.L.Spell then
			name = Quiver.L.Spell[fallbackEnglish]
		end
		if type(name) ~= "string" or name == "" then name = fallbackEnglish end
	end
	if type(name) == "string" and name ~= "" then
		localizedNameCache[spellId] = name
		if fallbackEnglish ~= nil then localizedToEnglishCache[name] = fallbackEnglish end
		return name
	end
	return nil
end
...
---@return nil|string
local GetLocalizedNameByEnglish = function(english)
	local spellId = ENGLISH_TO_ID[english]
	if spellId == nil then
		return Quiver and Quiver.L and Quiver.L.Spell and Quiver.L.Spell[english] or english
	end
	return GetLocalizedName(spellId, english)
end

---@param localized string
---@return nil|string
local GetEnglishName = function(localized)
	if localized == nil then return nil end
	local cached = localizedToEnglishCache[localized]
	if cached ~= nil then return cached end
	for english, spellId in pairs(ENGLISH_TO_ID) do
		local current = GetLocalizedName(spellId, english)
		if current == localized then
			localizedToEnglishCache[localized] = english
			return english
		end
	end
	return nil
...
local FindSpellIndex = function(spellName)
	...
	for spellIndex=1, numSpells do
		...
		if name == spellName then return spellIndex end
	end
	return nil
end
...
local PredInstantCast = function(name)
	...
	return meta == nil or meta.Haste == "none"
end
...
---@param fallbackEnglish? string
---@return boolean
local PredSpellLearnedById = function(spellId, fallbackEnglish)
	return PredSpellLearned(GetLocalizedName(spellId, fallbackEnglish))
end
...
local CheckNewCd = function(cooldown, lastCdStart, spellName)
	...
	if spellIndex ~= nil then
		...
		if durationCD == cooldown and timeStartCD ~= lastCdStart then return true, timeStartCD end
	end
	return false, lastCdStart
end

local CheckNewGCD = function(lastCdStart)
	return CheckNewCd(1.5, lastCdStart, GetLocalizedName(ID.SerpentSting, "Serpent Sting"))
end
...
return {
	...
	Db = DB_SPELL,
	Id = ID,
	EnglishToId = ENGLISH_TO_ID,
	FindSpellIndex = FindSpellIndex,
	GetEnglishName = GetEnglishName,
	GetLocalizedName = GetLocalizedName,
	GetLocalizedNameByEnglish = GetLocalizedNameByEnglish,
	PredInstantCast = PredInstantCast,
	PredInstantShot = PredInstantShot,
	PredSpellLearned = PredSpellLearned,
	PredSpellLearnedById = PredSpellLearnedById,
}
end)
__bundle_register("Api/Pet.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local L = require("Lib/Index.pure.lua")
...
__bundle_register("Api/Enum.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
...
__bundle_register("Api/Aura.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local ApiSpell = require "Api/Spell.wow.lua"
local Const = require "Constants.pure.lua"
local Diag = require "Diagnostics.wow.lua"

local queryHelpfulByName = function(name)
	if type(name) ~= "string" or name == "" then return nil end
	return C_UnitAuras.GetAuraDataBySpellName("player", name, "HELPFUL")
end

local decodeAura = function(aura, traceName)
	if aura == nil then
		Diag.Trace("AURA_QUERY", { name=traceName, active=false })
		return false, 0
	end
	local expiration = tonumber(aura.expirationTime) or 0
	local timeLeft = expiration > 0 and math.max(0, expiration - GetTime()) or 0
	Diag.Trace("AURA_QUERY", { name=traceName, active=true, spellId=aura.spellId, expiration=expiration, timeLeft=timeLeft })
...
local GetIsActiveAndTimeLeftByName = function(buffname)
	return decodeAura(queryHelpfulByName(buffname), buffname)
end
...
---@return boolean isActive
---@return number secondsLeft
local GetIsActiveAndTimeLeftBySpellId = function(spellId, fallbackEnglish)
	local name = ApiSpell.GetLocalizedName(spellId, fallbackEnglish)
	return decodeAura(queryHelpfulByName(name), name or fallbackEnglish or tostring(spellId))
end

-- Optional legacy helper retained for old internal code. It is not part of the
-- startup contract; when the richer index API is unavailable it fails closed.
local GetIsActiveAndTimeLeftByTexture = function(targetTexture)
	...
	for i=1,Const.Aura_Cap do
		local aura = C_UnitAuras.GetAuraDataByIndex("player", i, "HELPFUL")
		if aura == nil then break end
		if aura.icon == targetTexture then
			local expiration = tonumber(aura.expirationTime) or 0
			local timeLeft = expiration > 0 and math.max(0, expiration - GetTime()) or 0
			return true, timeLeft
		end
...
local PredBuffActive = function(buffname)
	local aura = queryHelpfulByName(buffname)
	local active = aura ~= nil
	Diag.Trace("AURA_ACTIVE", { name=buffname, active=active, spellId=active and aura.spellId or nil })
	return active
end
...
---@param fallbackEnglish? string
---@return boolean
local PredBuffActiveBySpellId = function(spellId, fallbackEnglish)
	local name = ApiSpell.GetLocalizedName(spellId, fallbackEnglish)
	return PredBuffActive(name)
end

return {
	GetIsActiveAndTimeLeftByName = GetIsActiveAndTimeLeftByName,
	GetIsActiveAndTimeLeftBySpellId = GetIsActiveAndTimeLeftBySpellId,
	GetIsActiveAndTimeLeftByTexture = GetIsActiveAndTimeLeftByTexture,
	PredBuffActive = PredBuffActive,
	PredBuffActiveBySpellId = PredBuffActiveBySpellId,
}
end)
__bundle_register("Api/Aero.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
...
__bundle_register("Api/Action.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local _MAX_NUM_ACTION_SLOTS = 120

-- ClassicAPI exposes the real action descriptor, including spell IDs. This avoids
-- Quiver's old texture-equality heuristic (which confused Aimed Shot with items
-- that share its icon).
---@param name string
...
local FindBySpellName = function(name)
	for i=1,_MAX_NUM_ACTION_SLOTS do
		...
		if actionType == "spell" and actionId ~= nil then
			...
			if spellName == name then return i end
		end
	end
	return nil
end
...
local PredSomeActionBusy = function()
	for i=1,_MAX_NUM_ACTION_SLOTS do
		if IsCurrentAction(i) then return 1 end
	end
	return nil
end
...
__bundle_register("Modules/BorderStyle.state.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Diag = require "Diagnostics.wow.lua"

---@alias BorderStyle "Simple" | "Tooltip"

---@type table<string, fun(x: BorderStyle): nil>
local callbacks = {}
...
return {
	...
	ChangeAndPublish = function(style)
		Diag.Trace("BORDER_STYLE", { style=style })
		Quiver_Store.Border_Style = style
		-- callbacks is keyed by module ID; ipairs silently skipped all entries.
		for _id, callback in pairs(callbacks) do callback(style) end
	end,
...
__bundle_register("Events/Spellcast.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Api = require "Api/Index.wow.lua"
local Diag = require "Diagnostics.wow.lua"

-- Optional ClassicAPI spellcast bus. Importing this module is side-effect free:
-- custom events are not registered until Initialize() is called after the
-- ClassicAPI runtime contract has passed.
...
local publish = function(callbacks, nameEnglish, nameLocalized, guid, spellId)
	for _k, callback in pairs(callbacks) do callback(nameEnglish, nameLocalized, guid, spellId) end
end

local CastableShot = {
	Subscribe = function(moduleId, callback) callbacksCastableShot[moduleId] = callback end,
	Dispose = function(moduleId) callbacksCastableShot[moduleId] = nil end,
}
local Instant = {
	Subscribe = function(moduleId, callback) callbacksInstant[moduleId] = callback end,
	Dispose = function(moduleId) callbacksInstant[moduleId] = nil end,
}

local handleEvent = function()
	if arg1 ~= "player" then return end
	local guid, spellId, nameLocalized = arg2, arg3, arg4
	if nameLocalized == nil then return end
	local nameEnglish = Api.Spell.GetEnglishName(nameLocalized)
	Diag.Trace("SPELLCAST_BUS", { event=event, guid=guid, spellId=spellId, localized=nameLocalized, english=nameEnglish })
	if event == "UNIT_SPELLCAST_START" then
		local meta = nameEnglish and Api.Spell.Db[nameEnglish] or nil
		if meta ~= nil and not Api.Spell.PredInstantCast(nameEnglish) then
			publish(callbacksCastableShot, nameEnglish, nameLocalized, guid, spellId)
		end
	elseif event == "UNIT_SPELLCAST_SUCCEEDED" then
		publish(callbacksInstant, nameEnglish, nameLocalized, guid, spellId)
	end
end

local Initialize = function()
	if initialized then return true end
	if type(CLASSIC_API_VERSION) ~= "number" or CLASSIC_API_VERSION < 11400 then return false end
	eventFrame = eventFrame or CreateFrame("Frame", nil)
	local okStart = pcall(eventFrame.RegisterEvent, eventFrame, "UNIT_SPELLCAST_START")
	local okSuccess = pcall(eventFrame.RegisterEvent, eventFrame, "UNIT_SPELLCAST_SUCCEEDED")
	if not okStart or not okSuccess then
		Diag.TraceAlways("SPELLCAST_BUS_INIT_FAILED", { start=okStart, success=okSuccess })
		return false
	end
	eventFrame:SetScript("OnEvent", handleEvent)
	initialized = true
	Diag.Trace("SPELLCAST_BUS_INIT", {})
	return true
end
...
__bundle_register("Events/FrameLock.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
...
__bundle_register("Component/Button.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Api = require "Api/Index.wow.lua"
local Util = require "Component/_Util.wow.lua"
local L = require "Lib/Index.pure.lua"
local Diag = require "Diagnostics.wow.lua"
...
function QqButton:OnMouseDown()
	if not self.isEnabled then
		...
		Diag.Trace("BUTTON_DISABLED_MOUSEDOWN_BLOCKED", { hasHook=self.HookMouseDown ~= nil })
		self:resetTexture()
		return
	end
...
function QqButton:OnMouseUp()
	self.isMouseDown = false
	if not self.isEnabled then
		Diag.Trace("BUTTON_DISABLED_CLICK_BLOCKED", { hasHook=self.HookClick ~= nil })
		self:resetTexture()
		return
	end
	if self.HookMouseUp ~= nil then self.HookMouseUp() end
	if MouseIsOver(self.Container) == 1 and self.HookClick ~= nil then self.HookClick() end
	self:resetTexture()
end
...
__bundle_register("Component/_Util.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Api = require("Api/Index.wow.lua")
local L = require("Lib/Index.pure.lua")
...
__bundle_register("Api/Config.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
-- CVars are strings on the 1.12 client. Comparing useUiScale to numeric 1
-- makes saved frame coordinates wrong on scaled UIs.
local getUiScale = function()
	return tonumber(GetCVar("useUiScale")) == 1 and UIParent:GetEffectiveScale() or 1
end
local GetScreenWidthScaled = function()
	return GetScreenWidth() * getUiScale()
end
local GetScreenHeightScaled = function()
	return GetScreenHeight() * getUiScale()
end
...
__bundle_register("Modules/UpdateNotifier.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Version = require("Util/Version.wow.lua")
...
__bundle_register("Util/Version.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
---@class (exact) Version
...
__bundle_register("Modules/TrueshotAuraAlarm.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Api=require "Api/Index.wow.lua"
local Const=require "Constants.pure.lua"
local FrameLock=require "Events/FrameLock.wow.lua"
local Diag=require "Diagnostics.wow.lua"
...
local aura=(function()
	...
	local textureFull=function() frame.Icon:SetAlpha(0.75); frame:SetBackdropBorderColor(1,0,0,0.8) end
	local textureOff=function() updateDelay=UPDATE_DELAY_SLOW; frame.Icon:SetAlpha(0); frame:SetBackdropBorderColor(0,0,0,0) end
	local texturePartial=function() frame.Icon:SetAlpha(0.4); frame:SetBackdropBorderColor(0,0,0,0.1) end
	return {
		ShouldUpdate=function(elapsed) lastUpdate=lastUpdate+elapsed; return knowsAura and lastUpdate>updateDelay end,
		UpdateUI=function()
			knowsAura=Api.Spell.PredSpellLearnedById(SID.TrueshotAura,"Trueshot Aura")
			isActive,timeLeft=Api.Aura.GetIsActiveAndTimeLeftBySpellId(SID.TrueshotAura,"Trueshot Aura"); lastUpdate=0
			Diag.Trace("TRUESHOT_UI", { knows=knowsAura, active=isActive, timeLeft=timeLeft, taxi=UnitOnTaxi("player") and true or false, locked=Quiver_Store.IsLockedFrames, updateDelay=updateDelay })
			if not Quiver_Store.IsLockedFrames then textureFull()
			elseif UnitOnTaxi("player") then textureOff()
			elseif knowsAura and not isActive then textureFull()
			elseif knowsAura and isActive and timeLeft>0 and timeLeft<MINUTES_LEFT_WARNING*60 then texturePartial()
			else textureOff() end
		end,
	}
end)()
local setFramePosition=function(f,s) FrameLock.SideEffectRestoreSize(s,{w=DEFAULT_ICON_SIZE,h=DEFAULT_ICON_SIZE,dx=150,dy=40}); f:SetWidth(s.FrameMeta.W); f:SetHeight(s.FrameMeta.H); f:SetPoint("TopLeft",s.FrameMeta.X,s.FrameMeta.Y) end
local createUI=function()
	local f=CreateFrame("Frame",nil,UIParent); f:SetFrameStrata("LOW"); f:SetBackdrop({edgeFile="Interface/Tooltips/UI-Tooltip-Border",edgeSize=20}); setFramePosition(f,store)
	f.Icon=f:CreateTexture(nil,"BACKGROUND"); f.Icon:SetTexture(Const.Icon.TrueshotAura); f.Icon:SetPoint("Left",f,"Left",INSET,0); f.Icon:SetPoint("Right",f,"Right",-INSET,0); f.Icon:SetPoint("Top",f,"Top",0,-INSET); f.Icon:SetPoint("Bottom",f,"Bottom",0,INSET)
	FrameLock.SideEffectMakeMoveable(f,store); FrameLock.SideEffectMakeResizeable(f,store,{GripMargin=0}); return f
end
local _EVENTS={"PLAYER_AURAS_CHANGED","UNIT_FLAGS","SPELLS_CHANGED","UNIT_SPELLCAST_SUCCEEDED"}
local handleEvent=function()
	if event=="UNIT_SPELLCAST_SUCCEEDED" then
		if arg1~="player" then return end
		local localized=Api.Spell.GetLocalizedName(SID.TrueshotAura,"Trueshot Aura")
		if arg3==SID.TrueshotAura or (localized~=nil and arg4==localized) then
			updateDelay=UPDATE_DELAY_FAST
			Diag.Trace("TRUESHOT_CAST_SUCCESS", { guid=arg2, spellId=arg3, name=arg4 })
		end
	elseif event=="SPELLS_CHANGED" and arg1~="LeftButton" or event=="PLAYER_AURAS_CHANGED" or event=="UNIT_FLAGS" and arg1=="player" then
		aura.UpdateUI()
	end
end
local onEnable=function()
	if frame==nil then frame=createUI() end
	frame:SetScript("OnEvent",handleEvent); frame:SetScript("OnUpdate",function() if aura.ShouldUpdate(arg1 or 0) then aura.UpdateUI() end end)
	for _i,v in ipairs(_EVENTS) do frame:RegisterEvent(v) end
	frame:Show(); aura.UpdateUI()
end
local onDisable=function() if frame then frame:Hide(); for _i,v in ipairs(_EVENTS) do frame:UnregisterEvent(v) end end end
return {
	Id=MODULE_ID,GetName=function() return Quiver.T["Trueshot Aura Alarm"] end,GetTooltipText=function() return nil end,
	OnEnable=onEnable,OnDisable=onDisable,OnInterfaceLock=function() aura.UpdateUI() end,OnInterfaceUnlock=function() aura.UpdateUI() end,
	OnResetFrames=function() store.FrameMeta=nil; if frame then setFramePosition(frame,store) end end,
	OnSavedVariablesRestore=function(savedVariables) store=savedVariables end,OnSavedVariablesPersist=function() return store end,
}
end)
__bundle_register("Modules/TranqAnnouncer.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Api = require "Api/Index.wow.lua"
local FrameLock = require "Events/FrameLock.wow.lua"
local BorderStyle = require "Modules/BorderStyle.state.lua"
local L = require "Lib/Index.pure.lua"
...
local SID=Api.Spell.Id
local lastOwnTranqAt=-100

local message=(function()
	local ADDON_MESSAGE_CAST="Quiver_Tranq_Shot"
	local MATCH="^"..ADDON_MESSAGE_CAST..":([^:]+):(%d+)$"
	return {
		Broadcast=function()
			if GetNumRaidMembers() <= 0 then return end
			local playerName=UnitName("player")
			local _,_,msLatency=GetNetStats()
			local serialized=ADDON_MESSAGE_CAST..":"..playerName..":"..tostring(tonumber(msLatency) or 0)
			SendAddonMessage("Quiver",serialized,"RAID")
		end,
		Deserialize=function(msg)
			if type(msg)~="string" then return nil,nil end
			local _,_,nameCaster,latencyText=string.find(msg,MATCH)
			if nameCaster==nil then return nil,nil end
			local msLatencyCaster=tonumber(latencyText) or 0
			local _,_,msLatency=GetNetStats()
			local timeCastSec=GetTime()-((tonumber(msLatency) or 0)+msLatencyCaster)/1000
...
local getColorForeground=(function()
	local NUM_COLORS=17
	local COLOR_FG={{0.95,0.05,0.05},{0.91,0.19,0.0},{0.79,0.35,0.08},{0.75,0.38,0.02},{0.72,0.40,0.0},{0.68,0.43,0.0},{0.64,0.45,0.0},{0.60,0.46,0.0},{0.56,0.48,0.0},{0.52,0.49,0.0},{0.48,0.51,0.0},{0.44,0.52,0.04},{0.40,0.53,0.10},{0.29,0.55,0.0},{0.23,0.55,0.0},{0.15,0.56,0.11},{0.00,0.56,0.18}}
	return function(progress)
		local p=progress<=0 and 0.001 or progress>=1 and 0.999 or progress
		return unpack(COLOR_FG[math.ceil(p*NUM_COLORS)])
	end
end)()
...
local createProgressBar=function()
	...
	bar:SetBackdrop({bgFile="Interface/BUTTONS/WHITE8X8",edgeFile="Interface/BUTTONS/WHITE8X8",edgeSize=1,tile=false})
	bar:SetBackdropBorderColor(0,0,0,0.6)
	local centerVertically=function(ele) ele:SetPoint("Top",bar,"Top",0,-BORDER_BAR); ele:SetPoint("Bottom",bar,"Bottom",0,BORDER_BAR) end
	bar.ProgressFrame=CreateFrame("Frame",nil,bar); centerVertically(bar.ProgressFrame); bar.ProgressFrame:SetPoint("Left",bar,"Left",BORDER_BAR,0)
	bar.ProgressFrame:SetBackdrop({bgFile="Interface/BUTTONS/WHITE8X8",tile=false})
	bar.FsPlayerName=bar.ProgressFrame:CreateFontString(nil,"ARTWORK","GameFontNormal"); centerVertically(bar.FsPlayerName); bar.FsPlayerName:SetPoint("Left",bar,"Left",MARGIN_TEXT,0); bar.FsPlayerName:SetJustifyH("Left"); bar.FsPlayerName:SetJustifyV("Center"); bar.FsPlayerName:SetTextColor(1,1,1)
	bar.FsCdTimer=bar.ProgressFrame:CreateFontString(nil,"ARTWORK","GameFontNormal"); centerVertically(bar.FsCdTimer); bar.FsCdTimer:SetPoint("Right",bar,"Right",-MARGIN_TEXT,0); bar.FsCdTimer:SetJustifyH("Right"); bar.FsCdTimer:SetJustifyV("Center"); bar.FsCdTimer:SetTextColor(1,1,1)
	return bar
end
local poolProgressBar=(function()
	...
	return {
		Acquire=function(parent)
			local bar=table.remove(fs) or createProgressBar(); bar:SetParent(parent)
			bar:SetFrameStrata("LOW"); bar.ProgressFrame:SetFrameStrata("MEDIUM"); bar:Show(); return bar
		end,
		Release=function(bar) bar:SetParent(nil); bar:ClearAllPoints(); table.insert(fs,bar) end,
	}
end)()

local getIdealFrameHeight=function()
	local heightBars=L.Array.MapReduce(frame.Bars,function(x) return x:GetHeight() end,L.M.Add)
	return math.max(heightBars,HEIGHT_BAR)+2*INSET
end
local adjustBarYOffsets=function()
	local height=0
	for _i,v in ipairs(frame.Bars) do
		v:ClearAllPoints(); v:SetPoint("Left",frame,"Left",INSET,0); v:SetPoint("Right",frame,"Right",-INSET,0); v:SetPoint("Top",frame,"Top",0,-height-INSET); height=height+v:GetHeight()
	end
end
local setFramePosition=function(f,s)
	FrameLock.SideEffectRestoreSize(s,{w=WIDTH_FRAME_DEFAULT,h=HEIGHT_BAR+2*INSET,dx=110,dy=150})
	f:SetWidth(s.FrameMeta.W); f:SetHeight(s.FrameMeta.H); f:SetPoint("TopLeft",s.FrameMeta.X,s.FrameMeta.Y)
end
local createUI=function()
	frame=CreateFrame("Frame",nil,UIParent); frame.Bars={}; frame:SetFrameStrata("LOW")
	frame:SetBackdrop({edgeFile="Interface/Tooltips/UI-Tooltip-Border",edgeSize=16,insets={left=INSET,right=INSET,top=INSET,bottom=INSET}}); frame:SetBackdropBorderColor(BorderStyle.GetColor())
	setFramePosition(frame,store); FrameLock.SideEffectMakeMoveable(frame,store); FrameLock.SideEffectMakeResizeable(frame,store,{GripMargin=4}); return frame
end

local getCanHide=function()
	local now=GetTime()
	return not UnitAffectingCombat("player") and L.Array.Every(frame.Bars,function(v) return now-v.TimeCastSec>=TRANQ_CD_SEC end) and Quiver_Store.IsLockedFrames
end
local hideFrameDeleteBars=function() frame:Hide(); for _i,v in ipairs(frame.Bars) do poolProgressBar.Release(v) end; frame.Bars={} end
local handleUpdate=function()
	if getCanHide() then hideFrameDeleteBars() end
	local now=GetTime()
	for _i,v in ipairs(frame.Bars) do
		local secElapsed=math.max(0,now-v.TimeCastSec); local secProgress=math.min(secElapsed,TRANQ_CD_SEC); local percentProgress=secProgress/TRANQ_CD_SEC
		v.ProgressFrame:SetWidth(math.max(1,(v:GetWidth()-2*BORDER_BAR)*percentProgress)); v.FsCdTimer:SetText(string.format("%.1f / %.0f",secProgress,TRANQ_CD_SEC))
		local r,g,b=getColorForeground(percentProgress); local s=0.7; v:SetBackdropColor(r*s,g*s,b*s,0.8); v.ProgressFrame:SetBackdropColor(r,g,b,0.9)
	end
end

local recordCast=function(nameCaster,timeCastSec)
	if nameCaster==nil or timeCastSec==nil then return end
	local barVisible=L.Array.Find(frame.Bars,function(bar) return bar.FsPlayerName:GetText()==nameCaster end)
	if barVisible then barVisible.TimeCastSec=timeCastSec
	else
		local barNew=poolProgressBar.Acquire(frame); barNew.TimeCastSec=timeCastSec; barNew:SetHeight(HEIGHT_BAR); barNew.FsPlayerName:SetText(nameCaster); table.insert(frame.Bars,barNew)
	end
	table.sort(frame.Bars,function(a,b) return a.TimeCastSec<b.TimeCastSec end); adjustBarYOffsets(); frame:SetHeight(getIdealFrameHeight()); frame:Show()
end

local announceConfigured=function(text,reason)
	local channel=store.TranqChannel
	Diag.Trace("TRANQ_ANNOUNCE", { channel=channel, reason=reason, text=text })
	if channel=="/Say" then
		Print.Line.Say(text)
	elseif channel=="/Raid" then
		-- Avoid sending an invalid RAID chat message while solo/party.
		if GetNumRaidMembers()>0 then Print.Line.Raid(text) end
	end
end

local announceCast=function()
	lastOwnTranqAt=GetTime()
	Diag.Trace("TRANQ_CAST", { channel=store.TranqChannel })
	recordCast(UnitName("player"),lastOwnTranqAt)
	message.Broadcast()
	announceConfigured(store.MsgTranqCast,"cast")
end

local patternEscape=function(text)
	return string.gsub(text,"([%(%)%.%%%+%-%*%?%[%]%^%$])","%%%1")
end
local formatToPattern=function(fmt)
	if type(fmt)~="string" or fmt=="" then return nil end
	local token="QQWILDCARDQQ"
	-- WoW localized GlobalStrings may include grammatical-selection markup such
	-- as |1...;...;. The rendered combat line contains the chosen form, not the
	-- markup, so treat the whole construct as a wildcard.
	local x=string.gsub(fmt,"|%d[^;]*;[^;]*;",token)
	-- Replace printf placeholders (%s, %d, %1$s, ...) with wildcards, then
	-- escape the remaining literal text as a Lua pattern.
	x=string.gsub(x,"%%%d*%$?[sd]",token)
	x=patternEscape(x)
	x=string.gsub(x,token,".-")
	return "^"..x.."$"
end
local failureFormats=function()
	return {
		SPELLMISSSELFOTHER, SPELLRESISTSELFOTHER, SPELLIMMUNESELFOTHER,
		SPELLLOGABSORBSELFOTHER, DISPELFAILEDSELFOTHER,
		SPELLDODGEDSELFOTHER, SPELLEVADEDSELFOTHER, SPELLPARRIEDSELFOTHER,
		SPELLBLOCKEDSELFOTHER, SPELLDEFLECTEDSELFOTHER, SPELLREFLECTSELFOTHER,
	}
end
local isLikelyTranqFailure=function(text)
	if type(text)~="string" or GetTime()-lastOwnTranqAt>3.0 then return false end
	-- A nearby unrelated spell can also miss/resist inside this three-second
	-- window. Require the actual localized Tranq spell name in the combat line
	-- before applying the generic localized GlobalString patterns.
	local tranqName=Api.Spell.GetLocalizedName(SID.TranquilizingShot,"Tranquilizing Shot")
	if type(tranqName)~="string" or not string.find(text,tranqName,1,true) then return false end
	for _i,fmt in pairs(failureFormats()) do
		local pat=formatToPattern(fmt)
		if pat and string.find(text,pat) then
			Diag.Trace("TRANQ_FAILURE_GLOBAL_MATCH", { format=fmt, text=text })
			return true
		end
	end
	-- Fallback for clients/servers whose combat line differs from stock
	-- GlobalStrings. Restrict it to the just-cast Tranq window to avoid the old
	-- false positive where an unrelated dispel failure triggered the warning.
	local legacy=Quiver.L and Quiver.L.CombatLog and Quiver.L.CombatLog.Tranq
	return legacy and ((legacy.Miss and string.find(text,legacy.Miss,1,true)) or (legacy.Resist and string.find(text,legacy.Resist,1,true)) or (legacy.Fail and string.find(text,legacy.Fail,1,true)))~=nil
end

local _EVENTS={"CHAT_MSG_ADDON","CHAT_MSG_SPELL_SELF_DAMAGE","UNIT_SPELLCAST_SUCCEEDED"}
local handleEvent=function()
	if event=="CHAT_MSG_ADDON" then
		if arg1~="Quiver" then return end
		local nameCaster,timeCastSec=message.Deserialize(arg2)
		if nameCaster~=UnitName("player") then recordCast(nameCaster,timeCastSec) end
	elseif event=="CHAT_MSG_SPELL_SELF_DAMAGE" then
		if isLikelyTranqFailure(arg1) then
			Diag.Trace("TRANQ_MISS_PATH", { channel=store.TranqChannel, text=arg1 })
			announceConfigured(store.MsgTranqMiss,"miss")
		end
	elseif event=="UNIT_SPELLCAST_SUCCEEDED" and arg1=="player" then
		Diag.Trace("TRANQ_SPELLCAST_SUCCESS_EVENT", { guid=arg2, spellId=arg3, name=arg4 })
		local tranqName=Api.Spell.GetLocalizedName(SID.TranquilizingShot,"Tranquilizing Shot")
		if arg3==SID.TranquilizingShot or (tranqName~=nil and arg4==tranqName) then announceCast() end
	end
end
local onEnable=function()
	if frame==nil then frame=createUI() end
	frame:SetScript("OnEvent",handleEvent); frame:SetScript("OnUpdate",handleUpdate); for _i,v in ipairs(_EVENTS) do frame:RegisterEvent(v) end
	if getCanHide() then hideFrameDeleteBars() else frame:Show() end
end
local onDisable=function() if frame then frame:Hide(); for _i,v in ipairs(_EVENTS) do frame:UnregisterEvent(v) end end end
return {
	Id=MODULE_ID, GetName=function() return Quiver.T["Tranq Shot Announcer"] end,
	...
	OnInterfaceLock=function() if getCanHide() then hideFrameDeleteBars() end end, OnInterfaceUnlock=function() frame:Show() end,
	OnResetFrames=function() store.FrameMeta=nil; if frame then setFramePosition(frame,store) end end,
	OnSavedVariablesRestore=function(savedVariables) store=savedVariables; store.MsgTranqMiss=savedVariables.MsgTranqMiss or Quiver.T["*** MISSED Tranq Shot ***"]; store.MsgTranqCast=store.MsgTranqCast or Quiver.T["Casting Tranq Shot"]; store.TranqChannel=store.TranqChannel or "/Say" end,
	OnSavedVariablesPersist=function() return store end,
}
end)
__bundle_register("Modules/RangeIndicator.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Api = require "Api/Index.wow.lua"
local Const = require "Constants.pure.lua"
local FrameLock = require "Events/FrameLock.wow.lua"
local Diag = require "Diagnostics.wow.lua"
...
local beastTypeName = nil
local lastRangeResult = {}
local lastRendered = nil
local SID = Api.Spell.Id

local getBeastTypeName = function()
	if beastTypeName == nil then
		local info = C_CreatureInfo.GetCreatureTypeInfo(1)
		beastTypeName = info and info.name or "Beast"
	end
	return beastTypeName
end

local setFramePosition = function(f, s)
	FrameLock.SideEffectRestoreSize(s, { w=190, h=35, dx=190 * -0.5, dy=-183 })
	f:SetWidth(s.FrameMeta.W); f:SetHeight(s.FrameMeta.H)
	f:SetPoint("TopLeft", s.FrameMeta.X, s.FrameMeta.Y)
end
local createUI = function()
	local f=CreateFrame("Frame",nil,UIParent); setFramePosition(f,store)
	FrameLock.SideEffectMakeMoveable(f,store); FrameLock.SideEffectMakeResizeable(f,store,{GripMargin=4})
	f:SetFrameStrata("LOW")
	f:SetBackdrop({bgFile="Interface/Tooltips/UI-Tooltip-Background",edgeFile="Interface/Tooltips/UI-Tooltip-Border",tile=true,tileSize=8,edgeSize=16,insets={left=4,right=4,top=4,bottom=4}})
	f:SetBackdropColor(0,0,0,0.6); f:SetBackdropBorderColor(0.25,0.25,0.25,1)
	local fs=f:CreateFontString(nil,"ARTWORK","GameFontNormal"); fs:SetAllPoints(f); fs:SetJustifyH("Center"); fs:SetJustifyV("Middle"); fs:SetText("Range Indicator"); fs:SetTextColor(1,1,1)
	return f,fs
end

-- ClassicAPI accepts numeric spell IDs directly. Using IDs avoids localization
-- and action-bar/macro dependencies. Wing Clip can therefore serve as the
-- melee-distance probe even before the character has learned it.
local predSpellInRange = function(spellId, label)
	local raw = C_Spell.IsSpellInRange(spellId,"target")
	if lastRangeResult[spellId] ~= raw then
		lastRangeResult[spellId] = raw
		Diag.Trace("RANGE_API", { spellId=spellId, spell=label, result=raw })
	end
	return raw == true
end
local checkDistance = {
	Follow=function() return CheckInteractDistance("target",4) end,
	Melee=function() return predSpellInRange(SID.WingClip,"Wing Clip") end,
	Mark=function() return predSpellInRange(SID.HuntersMark,"Hunter's Mark") end,
	Ranged=function() return predSpellInRange(SID.AutoShot,"Auto Shot") end,
	Scare=function() return predSpellInRange(SID.ScareBeast,"Scare Beast") end,
	Scatter=function() return predSpellInRange(SID.ScatterShot,"Scatter Shot") end,
}
local render=function(color,text)
	if lastRendered ~= text then
		lastRendered = text
		Diag.Trace("RANGE_RENDER", { text=text, targetType=UnitExists("target") and UnitCreatureType("target") or nil })
	end
	fontString:SetText(text); local r,g,b,a=unpack(color)
	frame:SetBackdropColor(r,g,b,a); frame:SetBackdropBorderColor(r,g,b,a)
end
local updateRange=function()
	...
	elseif checkDistance.Ranged() then
		if UnitCreatureType("target") == getBeastTypeName() and Api.Spell.PredSpellLearnedById(SID.ScareBeast,"Scare Beast") and checkDistance.Scare() then
			render(store.ColorScareBeast,Quiver.T["Scare Beast"])
		elseif Api.Spell.PredSpellLearnedById(SID.ScatterShot,"Scatter Shot") and checkDistance.Scatter() then render(store.ColorScatterShot,Quiver.T["Scatter Shot"])
		elseif checkDistance.Follow() then render(store.ColorShort,Quiver.T["Short Range"])
		else render(store.ColorLong,Quiver.T["Long Range"]) end
	elseif checkDistance.Follow() then render(store.ColorDeadZone,Quiver.T["Dead Zone"])
	elseif checkDistance.Mark() then render(store.ColorMark,Quiver.T["Hunter's Mark"])
	else render(store.ColorTooFar,Quiver.T["Out of Range"]) end
end
local handleUpdate=function()
	timeSinceUpdate=timeSinceUpdate+(arg1 or 0)
	if timeSinceUpdate>=UPDATE_INTERVAL then timeSinceUpdate=0; updateRange() end
end
local handleEvent=function()
	if (event=="UNIT_FACTION" or event=="UNIT_HEALTH") and arg1~="target" then return end
	if UnitExists("target") and not UnitIsDead("target") and UnitCanAttack("player","target") then frame:Show(); updateRange()
	elseif Quiver_Store.IsLockedFrames then frame:Hide() end
end
local _EVENTS={"PLAYER_TARGET_CHANGED","UNIT_FACTION","UNIT_HEALTH","SPELLS_CHANGED"}
local onEnable=function()
	if frame==nil then frame,fontString=createUI() end
	frame:SetScript("OnEvent",handleEvent); frame:SetScript("OnUpdate",handleUpdate)
	for _i,v in ipairs(_EVENTS) do frame:RegisterEvent(v) end
...
return {
	...
	GetTooltipText=function() return Quiver.T["Shows when abilities are in range."] end,
	OnEnable=onEnable,OnDisable=onDisable,
	OnInterfaceLock=function() handleEvent() end, OnInterfaceUnlock=function() frame:Show() end,
	OnResetFrames=function() store.FrameMeta=nil; if frame then setFramePosition(frame,store) end end,
	OnSavedVariablesRestore=function(savedVariables)
		store=savedVariables
...
__bundle_register("Modules/Castbar.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Api = require "Api/Index.wow.lua"
local L = require "Lib/Index.pure.lua"
local Const = require "Constants.pure.lua"
local FrameLock = require "Events/FrameLock.wow.lua"
...
local styleCastbar = function(f)
	...
	if BorderStyle.GetStyle() == "Tooltip" then
		...
		f:SetBackdrop({ bgFile="Interface/BUTTONS/WHITE8X8", edgeFile="Interface/BUTTONS/WHITE8X8", edgeSize=sizeInset })
		f:SetBackdropBorderColor(0.2, 0.2, 0.2, 0.8)
	end
	f:SetBackdropColor(0, 0, 0, 0.8)
	maxBarWidth = math.max(1, f:GetWidth() - 2 * sizeInset)
	...
	f.Castbar:SetHeight(math.max(1, f:GetHeight() - 2 * sizeInset))
	...
	f.SpellName:ClearAllPoints(); f.SpellTime:ClearAllPoints()
...
local setFramePosition = function(f, s)
	FrameLock.SideEffectRestoreSize(s, { w=240, h=20, dx=240 * -0.5, dy=-116 })
	f:SetWidth(s.FrameMeta.W); f:SetHeight(s.FrameMeta.H)
...
local createUI = function()
	...
	f.Castbar:SetBackdrop({ bgFile="Interface/BUTTONS/WHITE8X8" })
	f.SpellName = f.Castbar:CreateFontString(nil, "OVERLAY", "GameFontNormal")
	f.SpellName:SetJustifyH("Left"); f.SpellName:SetTextColor(1,1,1)
	f.SpellTime = f.Castbar:CreateFontString(nil, "OVERLAY", "GameFontNormal")
	f.SpellTime:SetJustifyH("Right"); f.SpellTime:SetTextColor(1,1,1)
	setFramePosition(f, store); styleCastbar(f)
	FrameLock.SideEffectMakeMoveable(f, store)
	FrameLock.SideEffectMakeResizeable(f, store, { GripMargin=4,
		OnResizeDrag=function() styleCastbar(f) end,
		OnResizeEnd=function() styleCastbar(f) end, IsCenterX=true })
	return f
end
...
local predTracked = function(localized)
	local english = Api.Spell.GetEnglishName(localized)
	local meta = english and Api.Spell.Db[english] or nil
	return meta ~= nil and not Api.Spell.PredInstantCast(english)
end

local refreshAuthoritativeTiming = function(expectedGuid)
	local name, _rank, _texture, startMs, endMs, _tradeskill, castGuid, _notInterruptible, spellId = C_Spell.UnitCastingInfo("player")
	Diag.Trace("CASTBAR_TIMING", { expectedGuid=expectedGuid, name=name, startMs=startMs, endMs=endMs, castGuid=castGuid, spellId=spellId, currentSpellId=currentSpellId })
	if name == nil or startMs == nil or endMs == nil then return false end
	if expectedGuid ~= nil and castGuid ~= expectedGuid then return false end
	if currentSpellId ~= nil and spellId ~= nil and currentSpellId ~= spellId then return false end
	currentCastGuid = castGuid or expectedGuid
	currentSpellId = spellId or currentSpellId
	timeStartCasting = startMs / 1000
	timeEndCasting = endMs / 1000
	castTime = math.max(0.001, timeEndCasting - timeStartCasting)
	return true
end

local beginCast = function(guid, spellId, nameLocalized)
	Diag.Trace("CASTBAR_BEGIN_REQUEST", { guid=guid, spellId=spellId, name=nameLocalized })
	currentCastGuid = guid
	currentSpellId = spellId
	if not refreshAuthoritativeTiming(guid) then
		Diag.Trace("CASTBAR_BEGIN_REJECT", { guid=guid, spellId=spellId, name=nameLocalized })
		currentCastGuid = nil; currentSpellId = nil; return
	end
	isCasting = true
	Diag.Trace("CASTBAR_BEGIN", { guid=currentCastGuid, spellId=currentSpellId, start=timeStartCasting, finish=timeEndCasting, castTime=castTime })
...
local finishCast = function(guid)
	if isCasting and guid == currentCastGuid then
		Diag.Trace("CASTBAR_FINISH", { guid=guid, spellId=currentSpellId })
		isCasting = false
		currentCastGuid = nil
		currentSpellId = nil
		frame.Castbar:SetWidth(1)
...
local handleUpdate = function()
	if not isCasting then frame.Castbar:SetWidth(1); return end
	local now = GetTime()
	local passed = now - timeStartCasting
	if passed <= castTime then
		displayTime(passed)
		frame.Castbar:SetWidth(math.max(1, maxBarWidth * passed / castTime))
	else
		displayTime(castTime)
		frame.Castbar:SetWidth(maxBarWidth)
		-- Backstop against a lost STOP event: authoritative casting state wins.
		if now > timeEndCasting + 0.25 then
			local _name, _, _, _, _, _, castGuid = C_Spell.UnitCastingInfo("player")
			if castGuid ~= currentCastGuid then finishCast(currentCastGuid) end
		end
	end
end
...
local handleEvent = function()
	if arg1 ~= "player" then return end
	Diag.Trace("CASTBAR_EVENT", { event=event, guid=arg2, spellId=arg3, name=arg4, activeGuid=currentCastGuid, isCasting=isCasting })
	if event == "UNIT_SPELLCAST_START" then
		if predTracked(arg4) then beginCast(arg2, arg3, arg4) end
	elseif event == "UNIT_SPELLCAST_DELAYED" then
		if isCasting and arg2 == currentCastGuid then refreshAuthoritativeTiming(arg2) end
	else
		-- FAILED spam while another cast is active has a different ClassicAPI
		-- cast GUID, so it cannot hide the real bar.
		finishCast(arg2)
	end
end
...
local onEnable = function()
	...
	frame:SetScript("OnEvent", handleEvent); frame:SetScript("OnUpdate", handleUpdate)
	for _i,v in ipairs(_EVENTS) do frame:RegisterEvent(v) end
	BorderStyle.Subscribe(MODULE_ID, function() if frame then styleCastbar(frame) end end)
end
local onDisable = function()
	BorderStyle.Dispose(MODULE_ID)
	isCasting=false; currentCastGuid=nil; currentSpellId=nil
	if frame then frame:Hide(); for _i,v in ipairs(_EVENTS) do frame:UnregisterEvent(v) end end
end
...
return {
	...
	OnResetFrames=function() store.FrameMeta=nil; if frame then setFramePosition(frame,store); styleCastbar(frame) end end,
	OnSavedVariablesRestore=function(savedVariables) store=savedVariables; store.ColorCastbar=store.ColorCastbar or Const.ColorDefault.Castbar end,
	OnSavedVariablesPersist=function() return store end,
}
end)
__bundle_register("Modules/Aspect_Tracker/AspectTracker.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Api = require "Api/Index.wow.lua"
local Const = require "Constants.pure.lua"
local FrameLock = require "Events/FrameLock.wow.lua"
local Diag = require "Diagnostics.wow.lua"
...
local isAura = function(spellId, english)
	return Api.Aura.PredBuffActiveBySpellId(spellId, english)
end

local chooseIconTexture = function()
	if isAura(SID.AspectBeast, "Aspect of the Beast") then
		return Const.Icon.Aspect_Beast
	elseif isAura(SID.AspectCheetah, "Aspect of the Cheetah") then
		return Const.Icon.Aspect_Cheetah
	elseif isAura(SID.AspectFox, "Aspect of the Fox") then
		return Const.Icon.Aspect_Fox
	elseif isAura(SID.AspectMonkey, "Aspect of the Monkey") then
		return Const.Icon.Aspect_Monkey
	elseif isAura(SID.AspectViper, "Aspect of the Viper") then
		return Const.Icon.Aspect_Viper
	elseif isAura(SID.AspectWild, "Aspect of the Wild") then
		return Const.Icon.Aspect_Wild
	elseif isAura(SID.AspectWolf, "Aspect of the Wolf") then
		return Const.Icon.Aspect_Wolf
	elseif Api.Spell.PredSpellLearnedById(SID.AspectHawk, "Aspect of the Hawk")
		and not isAura(SID.AspectHawk, "Aspect of the Hawk")
...
local updateUI = function()
	...
	local packActive = isAura(SID.AspectPack, "Aspect of the Pack")
	local alpha = packActive and 1.0 or 0.0
	frame:SetBackdropBorderColor(0.7, 0.8, 0.9, alpha)

	local onTaxi = UnitOnTaxi("player") and true or false
	if onTaxi and Quiver_Store.IsLockedFrames then frame:Hide() else frame:Show() end
	Diag.Trace("ASPECT_UI", {
		texture=activeTexture, pack=packActive, taxi=onTaxi,
		locked=Quiver_Store.IsLockedFrames, shown=frame:IsShown() and true or false,
	})
end

local setFramePosition = function(f, s)
	FrameLock.SideEffectRestoreSize(s, { w=DEFAULT_ICON_SIZE, h=DEFAULT_ICON_SIZE, dx=110, dy=40 })
	f:SetWidth(s.FrameMeta.W)
	f:SetHeight(s.FrameMeta.H)
	f:SetPoint("TopLeft", s.FrameMeta.X, s.FrameMeta.Y)
end
...
local _EVENTS = { "PLAYER_AURAS_CHANGED", "UNIT_FLAGS", "SPELLS_CHANGED" }
local handleEvent = function()
...
local onEnable = function()
	...
	frame:SetScript("OnEvent", handleEvent)
	for _i, v in ipairs(_EVENTS) do frame:RegisterEvent(v) end
	-- updateUI owns visibility. Do not unconditionally Show() afterward; doing
	-- so resurrected the tracker while the player was already on a taxi.
	updateUI()
end
local onDisable = function()
	if frame then frame:Hide() end
	for _i, v in ipairs(_EVENTS) do if frame then frame:UnregisterEvent(v) end end
end
...
return {
	...
	OnSavedVariablesRestore = function(savedVariables) store = savedVariables end,
	OnSavedVariablesPersist = function() return store end,
}
end)
__bundle_register("Migrations/Runner.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local M001 = require("Migrations/M001.wow.lua")
local M002 = require("Migrations/M002.wow.lua")
local M003 = require("Migrations/M003.wow.lua")
local Version = require("Util/Version.wow.lua")
...
__bundle_register("Migrations/M003.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local TranqAnnouncer = require("Modules/TranqAnnouncer.wow.lua")
...
__bundle_register("Migrations/M002.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local AutoShotTimer = require "Modules/Auto_Shot_Timer/AutoShotTimer.wow.lua"
...
return function()
	...
	if s.ColourReload then s.ColorReload = s.ColourReload end
	s.ColourShoot = nil
	s.ColourReload = nil
end
end)
__bundle_register("Migrations/M001.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local AutoShotTimer = require("Modules/Auto_Shot_Timer/AutoShotTimer.wow.lua")
...
__bundle_register("Locale/Lang.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
...
__bundle_register("Locale/zhCN/Translations.zhCN.pure.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
return {
...
__bundle_register("Locale/zhCN/Client.zhCN.pure.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Spell = require("Locale/zhCN/Spell.zhCN.pure.lua")
local SpellReverse = require("Locale/zhCN/Spell.reverse.zhCN.pure.lua")
-- local Zone = require "Locale/zhCN/Zone.zhCN.pure.lua"
...
__bundle_register("Locale/zhCN/Spell.reverse.zhCN.pure.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
return {
	["孤狼守护"] = "Aspect of the Wolf",
	["灵狐守护"] = "Aspect of the Fox",
...
__bundle_register("Locale/zhCN/Spell.zhCN.pure.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
return {
...
__bundle_register("Locale/enUS/Translations.enUS.pure.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
return {
	...
	["Quiver Unlocked.\nShow config dialog with /qq or /quiver.\nClick the lock icon when done."] = "Quiver Unlocked.\nShow config dialog with /qq or /quiver.\nClick the lock icon when done.",
...
__bundle_register("Locale/enUS/Client.enUS.pure.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
...
__bundle_register("Locale/enUS/Spell.enUS.pure.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
return {
	["Aspect of the Fox"] = "Aspect of the Fox",
...
__bundle_register("Config/MainMenu.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Api = require("Api/Index.wow.lua")
local Button = require("Component/Button.wow.lua")
local Dialog = require("Component/Dialog.wow.lua")
local IconButton = require("Component/IconButton.wow.lua")
local Select = require("Component/Select.wow.lua")
local Switch = require("Component/Switch.wow.lua")
local TitleBox = require("Component/TitleBox.wow.lua")
local Color = require("Config/Color.wow.lua")
local InputText = require("Config/InputText.wow.lua")
local Const = require("Constants.pure.lua")
local FrameLock = require("Events/FrameLock.wow.lua")
local L = require("Lib/Index.pure.lua")
local AutoShotTimer = require("Modules/Auto_Shot_Timer/AutoShotTimer.wow.lua")
...
__bundle_register("Config/InputText.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local EditBox = require("Component/EditBox.wow.lua")
local TranqAnnouncer = require("Modules/TranqAnnouncer.wow.lua")
...
__bundle_register("Component/EditBox.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Button = require("Component/Button.wow.lua")
local Const = require("Constants.pure.lua")
...
__bundle_register("Config/Color.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Button = require("Component/Button.wow.lua")
local ColorSwatch = require("Component/ColorSwatch.wow.lua")
local Const = require("Constants.pure.lua")
local L = require("Lib/Index.pure.lua")
local AutoShotTimer = require("Modules/Auto_Shot_Timer/AutoShotTimer.wow.lua")
local Castbar = require("Modules/Castbar.wow.lua")
...
__bundle_register("Component/ColorSwatch.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Button = require("Component/Button.wow.lua")
local Const = require("Constants.pure.lua")
...
__bundle_register("Component/TitleBox.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
---@param parent Frame
---@param text string
---@return Frame
...
__bundle_register("Component/Switch.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Api = require("Api/Index.wow.lua")
local Util = require("Component/_Util.wow.lua")
local Const = require("Constants.pure.lua")
local L = require("Lib/Index.pure.lua")
...
__bundle_register("Component/Select.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Api = require("Api/Index.wow.lua")
local Util = require("Component/_Util.wow.lua")
local Const = require("Constants.pure.lua")
local L = require("Lib/Index.pure.lua")
...
__bundle_register("Component/IconButton.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Util = require("Component/_Util.wow.lua")
...
__bundle_register("Component/Dialog.wow.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
local Const = require("Constants.pure.lua")
